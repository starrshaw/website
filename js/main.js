...
function renderPortfolioGrid() {
    ...
    filteredProjects.forEach((project, index) => {
        ...
        const thumbSrc = resolveImageSrc(project, thumb) || generatePortfolioPlaceholder(project.title);
...
async function loadProjectImages(project) {
    ...
    mediaEntries.forEach((entry) => {
        if (entry.type === 'image') {
            ...
            const urlCandidates = buildPortfolioImageUrlCandidates(project, entry.value);
            img.src = urlCandidates[0] || resolveImageSrc(project, entry.value);
            img.alt = `${project.title} - image ${imageIndex + 1}`;
            // Load all modal images eagerly — lazy loading leaves the gallery blank when opened.
            img.loading = 'eager';
            img.decoding = 'async';
            attachThumbnailFallback(img, project, [entry.value]);
...
function getSiteBasePath() {
    ...
    const websiteMatch = pathname.match(/^(.*\/website)(?=\/|$)/i);
    return websiteMatch ? websiteMatch[1] : '';
}

function resolvePublicAssetPath(pathValue) {
...
function buildAssetPath(folder, filename) {
    ...
    return resolvePublicAssetPath(
        `web_images/${encodeURIComponent(folder)}/${encodeURIComponent(filename)}`
    );
}
...
function resolveImageSrc(project, fileName) {
    ...
    if (normalizedValue.startsWith('/')) {
        return resolvePublicAssetPath(normalizedValue.slice(1));
    }

    if (normalizedValue.startsWith('web_images/')) {
        return resolvePublicAssetPath(normalizedValue);
    }
...
function generatePortfolioPlaceholder(title) {
    const safeTitle = String(title || 'Portfolio')
        .slice(0, 40)
        .replace(/&/g, '&amp;')
        .replace(/</g, '&lt;')
        .replace(/>/g, '&gt;')
        .replace(/"/g, '&quot;');
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="640" height="400" viewBox="0 0 640 400"><rect width="640" height="400" fill="#1a1a1a"/><text x="32" y="210" fill="#888" font-family="Arial,sans-serif" font-size="28">${safeTitle}</text></svg>`;
...
function buildPortfolioImageUrlCandidates(project, fileName) {
    const urls = [];
    const push = (url) => {
    ...
    push(resolveImageSrc(project, fileName));

    const value = String(fileName || '').trim();
    if (value.startsWith('web_images/')) {
        push(`${getSiteBasePath()}/api/image?path=${encodeURIComponent(value)}`);
        push(`/${value}`);
    } else if (project?.folder) {
        const joined = `web_images/${project.folder}/${value}`;
        push(`${getSiteBasePath()}/api/image?path=${encodeURIComponent(joined)}`);
    }
...
function attachThumbnailFallback(imgEl, project, candidates) {
    ...
    list.forEach((fileName) => {
        buildPortfolioImageUrlCandidates(project, fileName).forEach((url) => {
    ...
    let idx = Math.max(0, urlQueue.indexOf(imgEl.getAttribute('src') || imgEl.src || '') + 1);
    const tryNext = () => {
        if (idx >= urlQueue.length) {
            imgEl.src = generatePortfolioPlaceholder(project?.title || 'Portfolio');
            imgEl.style.backgroundColor = '#2a2a2a';
            return;
        }
        imgEl.src = urlQueue[idx];
        idx += 1;
    };
